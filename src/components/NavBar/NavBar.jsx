...
const NavBar = () => {
  ...
  return (
    <header className="flex justify-between items-center w-full px-4 h-20 text-white bg-[#141E30] fixed z-10">
      <div>
        <Link to='home' smooth duration={500} className="text-5xl font-signature ml-2 cursor-pointer">Yuliia</Link>
      </div>
      ...
      {nav && (
        <ul className="flex flex-col justify-center items-center absolute top-0 left-0 w-full h-screen bg-gradient-to-b from-[#141E30] to-[#243B55] text-gray-500">
          {links.map(({ id, link }) => {
            return (
              <li
                key={id}
